...
export const StatusBar = ({ page, walletBalance, uuid }) => {
  ...
  return (
    <div className="statusbar">
      ...
      <span
        className="search"
        style={{ visibility: page.search ? "visible" : "hidden" }}
      >
        <input className="searchInput" type="text" placeholder="Search" />
        <img src={search} alt="search icon" />
      </span>
      <p className="statusbar__text">
        Wallet Balance: {formatToCurrency(balance)}
      </p>
      <p className="statusbar__text statusbar__text__transaction-balance">
